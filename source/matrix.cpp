...
constexpr int MATRIX_FORWARD_ELIMINATION = 0;
constexpr int MATRIX_BACKWARD_ELIMINATION = 1;
extern unsigned char *epsilon;

int computeRank (int M, int Q, unsigned char **matrix)
{
   int i, rank, m = std::min (M, Q);
...
unsigned char **create_matrix (int M, int Q)
{
   ...
   if ((matrix = static_cast<unsigned char **> (std::calloc (M, sizeof (unsigned char *)))) == nullptr)
   {
      std::printf ("ERROR IN FUNCTION create_matrix:  Insufficient memory available.\n");
   ...
   else
   {
      for (i = 0; i < M; i++)
      {
         if ((matrix[i] = static_cast<unsigned char *> (std::calloc (Q, sizeof (unsigned char)))) == nullptr)
         {
            std::printf ("ERROR IN FUNCTION create_matrix: Insufficient memory for %dx%d matrix.\n", M, M);
